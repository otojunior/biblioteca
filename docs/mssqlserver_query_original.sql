...
SELECT procedimento.id,
	...
	(SELECT top 1 id
		FROM rl_atendimento_inconsistencia
		WHERE registro_ativo=1 AND fk_atendimento=obj.id) AS idInconsistencia,
	(SELECT top 1 id
		FROM rl_atendimento_tipo_pendencia
		WHERE registro_ativo=1 AND fk_atendimento=obj.id) AS idPendencia,
...
LEFT JOIN tb_tabela_tiss grauParticipacao ON (grauParticipacao.id = entidadeGrauParticipacao.fk_grau_participacaoAND grauParticipacao.registro_ativo = 1)
